...
function textureWith(channels: MaterialState['channels']): MaterialState {
  return { ...newMaterial(), channels }
}
...
describe('resetMaterial', () => {
  ...
  it('hands back a copy rather than the defaults themselves', () => {
    const reset = resetMaterial(newMaterial())
...
describe('parseMaterial', () => {
  ...
  it('reads back what an editor wrote, field by field', () => {
    ...
    expect(parseMaterial(JSON.parse(JSON.stringify(written)))).toEqual(written)
  })
  ...
  it('fills in what an older build never wrote', () => {
    const parsed = parseMaterial({ channels: {}, material: { roughness: 0.25 } })
  ...
  it('drops a channel a hand edit invented', () => {
    const parsed = parseMaterial({ channels: { glitter: { assetId: 'g', width: 8, height: 8 } } })
    expect(parsed.channels).toEqual({})
  })

  it('drops a channel entry with no asset behind it', () => {
    expect(parseMaterial({ channels: { normal: { origin: 'derived' } } }).channels).toEqual({})
    expect(parseMaterial({ channels: { normal: { assetId: '' } } }).channels).toEqual({})
  })
  ...
  it('files a channel of unknown origin as imported', () => {
    const parsed = parseMaterial({ channels: { ao: { assetId: 'a', origin: 'conjured' } } })
    expect(parsed.channels.ao?.origin).toBe('imported')
  })

  it('refuses a skybox environment that names no asset', () => {
    const parsed = parseMaterial({ preview: { environment: { kind: 'skybox' } } })
    expect(parsed.preview.environment).toEqual({ kind: 'studio' })
  })

  it('refuses a preview repeat the viewport cannot show', () => {
    expect(parseMaterial({ preview: { tilingPreview: 3 } }).preview.tilingPreview).toBe(1)
    expect(parseMaterial({ preview: { tilingPreview: 4 } }).preview.tilingPreview).toBe(4)
  })
  ...
  it('opens blank on what is not a texture at all', () => {
    expect(parseMaterial('a texture')).toEqual(newMaterial())
    expect(parseMaterial(null)).toEqual(newMaterial())
    expect(parseMaterial(42)).toEqual(newMaterial())
  })
  ...
  it('puts crossed remap handles back in order', () => {
    const parsed = parseMaterial({ material: { roughnessRange: { min: 0.9, max: 0.1 } } })
  ...
  it('leaves a signed setting its sign', () => {
    const parsed = parseMaterial({ material: { normalScale: -2, offset: { x: -0.5, y: 0.5 } } })
...
describe('the bounds a slider and a file share', () => {
  ...
  it('holds the low end too, where a negative relief is legitimate and a negative height is not', () => {
    const texture = parseMaterial({ material: { normalScale: -40, heightScale: -1 } })
  ...
  it('keeps a signed normal scale, which is the answer to a map baked the other way round', () => {
    expect(parseMaterial({ material: { normalScale: -1 } }).material.normalScale).toBe(-1)
  ...
  it('wraps a rotation instead of clamping it', () => {
    expect(parseMaterial({ material: { rotation: 100 } }).material.rotation).toBeCloseTo(
      100 % (Math.PI * 2),
    )
    expect(parseMaterial({ material: { rotation: -Math.PI / 2 } }).material.rotation).toBeCloseTo(
      (Math.PI * 3) / 2,
    )
  })

  it('wraps the rotation of the sky the same way', () => {
    expect(parseMaterial({ preview: { envRotation: 100 } }).preview.envRotation).toBeCloseTo(
      100 % (Math.PI * 2),
    )
  })
  ...
  it('holds a tiling above zero on both axes', () => {
    const texture = parseMaterial({ material: { tiling: { x: 0, y: -4 } } })
  ...
  it('holds the lighting inside what its slider can reach', () => {
    expect(parseMaterial({ preview: { envIntensity: 99 } }).preview.envIntensity).toBe(
      PREVIEW_BOUNDS.envIntensity.max,
    )
  })

  it('leaves the offset alone, which three wraps itself', () => {
    expect(parseMaterial({ material: { offset: { x: 1.5, y: -0.25 } } }).material.offset).toEqual({
      x: 1.5,
      y: -0.25,
    })
  })
