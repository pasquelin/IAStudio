...
/**
 * What `SceneRenderer.refreshGizmoMatrices` exists for, kept where a reader can rerun it.
 *
 * `TransformControls` turns its drag plane to face the eye for the axis being held, and it does so
 * in `updateMatrixWorld` — which only a RENDER calls. The studio draws on demand and a hover asks
 * for no frame, so without a push the plane keeps the orientation of the view one has quitted.
 * Measured in the running app on 2026-08-19: normal (0,-1,0) held over from « De dessus », giving
 * ray·normal 0 in « De gauche » and 0.0001 in « De face » — the handle lit up and the drag moved
 * nothing, in exactly those two panes.
 */
...
function rayAgainstPlane(controls: TransformControls, camera: OrthographicCamera): number {
  const normal = new Vector3(0, 0, 1).applyQuaternion(
    // Structural: the plane is the controls' own child, and its orientation is the whole subject.
    (controls as unknown as { _plane: Mesh })._plane.getWorldQuaternion(
      new Mesh().quaternion.clone(),
    ),
  )
