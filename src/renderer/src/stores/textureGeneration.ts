...
const landing = createGenerationLanding({
  ...
  land: (documentId, asset) => {
    void import('@/spaces/textures/placeChannel').then(({ placeTextureChannel }) =>
      placeTextureChannel(documentId, asset),
    )
  },
})

export const claimTextureOnSubmit = landing.claimOnSubmit
export const connectTextureGeneration = landing.connect
