...
const PLAN: OptimizationPlan = {
  classifications: [], instances: [], bakeCandidates: [], batches: [], merges: [],
  sharedGeometry: [], sharedMaterials: [], spatialCells: [], warnings: [],
  measured: {
    objects: 1, visibleObjects: 1, meshes: 1, draws: 1, triangles: 12, vertices: 24,
    geometryBytes: 96, textureBytes: 0, sharedMaterials: 0,
  },
  estimated: {
    drawCallsBefore: 1, drawCallsAfter: 1, avoidedGeometryBytes: 0, avoidedTextureBytes: 0,
  },
}
...
describe('optimizing the scene through the assistant', () => {
  it('uses the mounted analyzer for plans and measured reports', async () => {
    ...
    registerSceneEngine(DOCUMENT, {
      analyzeOptimization, analyzeWorldOptimization,
    ...
    expect(await runAction('optimization.report', {})).toMatchObject({
      ok: true, data: { visualChanges: 'NONE' },
    })
  ...
  it('analyzes authoring state without requiring a mounted viewport', async () => {
    ...
    expect(await runAction('optimization.analyze', {})).toMatchObject({
      ok: true, data: { measured: { objects: 1, meshes: 1 } },
    })
  })
  ...
  it('writes persisted overrides through undoable scene commands', async () => {
    ...
    installScene(DOCUMENT, {
      ...createDefaultScene(), nodes: [first, second], selectedIds: [first.id],
    })

    expect(await runAction('optimization.selection', {})).toMatchObject({ ok: true })
    expect(await runAction('optimization.exclude', { nodeIds: [second.id] })).toMatchObject({ ok: true })
    useScenes.getState().undo(DOCUMENT)
    expect(await runAction('optimization.setMode', {
      nodeIds: [first.id], mode: 'batch',
    })).toMatchObject({ ok: true, data: { mode: 'batch' } })
    expect(await runAction('optimization.world', {})).toMatchObject({ ok: true })
    expect(sceneOf(useScenes.getState(), DOCUMENT).nodes.map(node => node.optimization?.mode))
      .toEqual(['batch', 'auto'])
  })
