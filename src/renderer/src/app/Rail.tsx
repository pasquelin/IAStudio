 ...
 */
export function Rail({ cote, ouverts, surBascule }: RailProps) {
  const hautes: ZoneOutils[] = cote === 'gauche' ? ['gauche'] : ['droite']
  ...
  return (
    <div
      ...
      className={cn(
        'flex w-(--sc-rail) shrink-0 flex-col items-center justify-between py-1.5',
        cote === 'gauche' ? 'pl-1' : 'pr-1',
      )}
    >
      <GroupeRail zones={hautes} ouverts={ouverts} surBascule={surBascule} />
      <GroupeRail zones={basses} ouverts={ouverts} surBascule={surBascule} />
    </div>
  )
}
      ...
      {zones.flatMap(zone =>
        outilsDeZone(zone).map(outil => (
          <ToolButton
            ...
            infobulle={infobulle}
            actif={ouverts[zone] === outil.id}
            onClick={() => surBascule(zone, outil.id)}
            className="size-(--sc-rail) rounded-(--radius-sc-md)"
          />
        )),
      )}
    </div>
