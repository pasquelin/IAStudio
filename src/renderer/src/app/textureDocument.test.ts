...
function openMaterial(path = 'Matières/Murs/Brique.mtlx'): void {
  useDocuments.setState({
    documents: {
      [DOCUMENT]: { id: DOCUMENT, kind: 'texture', title: 'Brique', workspace: 'textures', path },
    },
    activeId: DOCUMENT,
  })
}

function withBaseColour(assetId: string): TextureState {
...
describe('the pictures a material points at', () => {
  it('writes each channel as a path relative to the document own folder', () => {
    expect(filesOf(texturePayload(withBaseColour('asset-brick'), DOCUMENT))).toEqual([
      '../../Assets/brick.png',
    ])
  })
  ...
  it('keeps the path the file carried when the catalogue answers nothing', () => {
    textureFromPayload(texturePayload(withBaseColour('asset-brick'), DOCUMENT), DOCUMENT)
    emptyCatalogue()
...
describe('a material that opened holding more than the studio composes', () => {
  ...
  it('refuses nothing for a file the studio composed whole', () => {
    textureFromPayload(texturePayload(newTexture(), DOCUMENT), DOCUMENT)
  ...
  it('refuses nothing for bytes that are not a material at all', () => {
    textureFromPayload({ not: 'a material' }, DOCUMENT)
