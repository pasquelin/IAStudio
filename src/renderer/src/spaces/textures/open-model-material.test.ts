...
import { openModelMaterial, type ChannelTexture } from './open-model-material'
...
describe('opening the material of a model', () => {
  ...
  it('puts every named map into the channel it plays, in one document', async () => {
    ...
    const channels = textureOf(useTextures.getState(), opened().id).channels
    ...
    expect(useLayouts.getState().activeWorkspace).toBe('textures')
  })
  ...
  it('comes back to the tab already assembled rather than making a second one', async () => {
    ...
    expect(textureOf(useTextures.getState(), first).channels.baseColor).toMatchObject({
      assetId: 'asset-base',
    })
  })
   ...
   * `openAsset` asks it: left to `placeTextureChannel`, which refuses one by one, the gesture
