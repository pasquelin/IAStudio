...
const inspected = () => inspectedChannel(useTextureViews.getState(), DOCUMENT)

const show = () => render(<TextureToolbar documentId={DOCUMENT} onFrame={onFrame} />)
...
describe('the texture bar', () => {
  ...
  it('turns the repeat preview up without touching what the material repeats', async () => {
    ...
    expect(textureOf(useTextures.getState(), DOCUMENT).material.tiling).toEqual({ x: 1, y: 1 })
  })
  ...
  it('leaves an undo entry behind, as the inspector does', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Sphère' }))
    useTextures.getState().undo(DOCUMENT)
