 ...
 *
 * The channels are placed through `placeMaterialChannel`, the same command a drop and the shelf
...
export async function openModelMaterial(
  ...
  for (const texture of placeable) placeMaterialChannel(created.id, texture, texture.map)
}
