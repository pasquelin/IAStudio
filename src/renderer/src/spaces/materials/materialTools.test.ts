...
import { DEFAULT_PREVIEW, PREVIEW_SHAPES, TILING_PREVIEWS } from '@/engines/material/materialState'
import {
  channelFrom,
  MATERIAL_MODE,
  nextIn,
...
const input = (overrides: Partial<Parameters<typeof materialTools>[0]> = {}) => ({
...
describe('the texture bar registry', () => {
  ...
  it('names nothing a bundle does not hold', () => {
    const keys = materialTools(input({ filled: PBR_CHANNELS })).flatMap(tool => [
      tool.labelKey,
  ...
  it('wears the shape in use, so the support is legible without opening anything', () => {
    const tools = materialTools(input({ preview: { ...DEFAULT_PREVIEW, shape: 'plane' } }))
  ...
  it('shows the three toggles as pressed exactly when they are on', () => {
    ...
    const tools = materialTools(input({ preview }))
