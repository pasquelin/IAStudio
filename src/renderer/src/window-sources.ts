 ...
 *
 * The main side has `main/sourceFiles.ts`, which walks the disk. This one cannot: the renderer
 ...
 *
 * **A guard importing this reads nothing itself, so `wideGuards.ts` has to recognise the import
...
export const WINDOW_SOURCES: Record<string, string> = import.meta.glob(
  [
    ...
    // suite asserts on and reaches no screen — the line `main/sourceFiles.ts` draws on its side.
    '!./**/*-fixtures.ts',
    '!./**/*-fixtures.tsx',
  ],
