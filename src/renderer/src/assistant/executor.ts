...
async function runCleared(
  ...
  if (cleared.quoted && !unchangedSince(cleared.quoted))
    return refused(
      'formChanged',
      'the generation form moved between the figure being quoted and the yes, and what was priced is what goes out — generator.armed says what stands now, then send this again',
    )
