...
describe('what a timeline is told to cue', () => {
  ...
  it('cues an event, and answers the id it gave it', async () => {
    const outcome = await runAction('timeline.cue', {
      list: 'events',
      at: 2_000_000,
      what: 'DoorOpened',
    })
  ...
  it('refuses a list nothing holds, and a transition of no known kind', async () => {
    expect(await runAction('timeline.cue', { list: 'markers', at: 0, what: 'x' })).toMatchObject({
      ok: false,
      refusal: 'badInput',
    })
    expect(
      await runAction('timeline.cue', { list: 'transitions', at: 0, what: 'iris', duration: 1 }),
  ...
  it('refuses a sound with no asset and one with no length', async () => {
    expect(
      await runAction('timeline.cue', { list: 'audio', at: 0, what: '', duration: 5 }),
    ).toMatchObject({ ok: false })
    expect(
      await runAction('timeline.cue', { list: 'audio', at: 0, what: 'music', duration: 0 }),
    ).toMatchObject({ ok: false })
  })

  it('takes a row back off, and refuses one that is not there', async () => {
    const held = await runAction('timeline.cue', { list: 'events', at: 0, what: 'Opened' })
    const id = held.ok ? (held.data as { id: string }).id : ''

    expect(await runAction('timeline.remove', { list: 'events', id })).toMatchObject({ ok: true })
    expect(timeline().events).toEqual([])
    expect(await runAction('timeline.remove', { list: 'events', id })).toMatchObject({ ok: false })
  })

  it('sets what the panel offers, and refuses setting it to what it already is', async () => {
    expect(await runAction('timeline.template', { template: 'intro' })).toMatchObject({ ok: true })
    expect(timeline().template).toBe('intro')
    expect(await runAction('timeline.template', { template: 'intro' })).toMatchObject({ ok: false })
  })
})
