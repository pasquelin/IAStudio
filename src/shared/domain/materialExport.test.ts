...
import { PBR_CHANNELS, type PbrChannel } from './material'
import {
  assetsOf,
  boundedSize,
  maxSizeOf,
  resolvePictures,
  MATERIAL_EXPORT_TARGETS,
  type ExportChannels,
...
const FOLDER_TARGETS: readonly MaterialExportTarget[] = ['unity', 'unreal', 'roblox', 'raw']
...
describe('the target registry', () => {
  ...
  it('resolves at least one picture for every target it lists', () => {
    for (const target of MATERIAL_EXPORT_TARGETS) {
...
describe('the remap of the material panel', () => {
  ...
  it('reaches every engine, not only the two the other cases visit', () => {
    for (const target of MATERIAL_EXPORT_TARGETS.filter(candidate => candidate !== 'raw')) {
...
describe('the size a target accepts', () => {
  it('caps Roblox at what the platform refuses above, and nothing else', () => {
    ...
    for (const target of MATERIAL_EXPORT_TARGETS.filter(target => target !== 'roblox')) {
