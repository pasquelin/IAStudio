...
/** One brush of the graph: a shape, where it stands, and the name it goes back to on separate. */
export type CsgPart = {
  name: string
...
/** What a solid is born as: one brush, nothing taken out of it yet. */
export function csgGraphOf(base: CsgPart): CsgGraph {
  return { base, steps: [], collision: 'trimesh' }
}

export function csgPartOf(name: string, geometry: GeometryDescriptor): CsgPart {
  return { name, geometry, transform: IDENTITY_TRANSFORM }
}
