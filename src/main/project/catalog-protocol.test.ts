...
/**
 * What this boundary must REFUSE. `catalog-client.test.ts` drives the shapes that pass — a
...
describe('telling the catalogue worker messages apart', () => {
  ...
  it('keeps what runs beside the queue out of it', () => {
    ...
    // Neither reaches this guard in production — `catalog-queue` asks `isRescan` and
