...
describe('the light theme', () => {
  ...
  it('actually changes them, rather than restating the dark value', () => {
    // Three are deliberately shared: the accent reads on either background, and the create
    // button keeps its warm colour in both themes.
    const shared = ['--color-accent', '--color-create', '--color-create-hover']
