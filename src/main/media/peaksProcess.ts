...
import { createPeaksClient, type PeaksClient, type PeaksPort } from './peaksClient'
import type { PeaksResponse } from './peaksProtocol'
...
export function openPeaksProcess(onExit: () => void = () => {}): PeaksClient {
  // Resolved beside the bundled main, where `peaksWorker` is its own entry point — see
  ...
  const child = utilityProcess.fork(fileURLToPath(new URL('./peaksWorker.js', import.meta.url)))
