# CHANGED by IA Studio, which resolves the two config ids to local folders. Upstream: https://github.com/wyysf-98/CraftsMan (branch
# CraftsMan-v1.5), under CreativeML Open RAIL-M — see vendor/CRAFTSMAN-LICENSE.
import random
import torch
...
# Set by the studio's adapter before the model is built: a repository id maps to the folder the
# manifest fetched its config into, so nothing here reaches the network.
LOCAL_CONFIGS: dict = {}


def _config_source(name: str) -> str:
    return LOCAL_CONFIGS.get(name, name)
...
class CondEmbedder(BaseEmbedder):
    ...
    def configure(self) -> None:
        ...
        if not self.cfg.encode_camera:
            if self.cfg.pretrained_clip_name_or_path is not None:
                ...
                self.clip_model: CLIPModel = CLIPModel(config=ConditionalCLIPModel.config_class.from_pretrained(
                    _config_source("openai/clip-vit-large-patch14"),
                ))
            if self.cfg.pretrained_dino_name_or_path is not None:
                ...
                self.dino_model: Dinov2Model = Dinov2Model(config=ConditionalDinov2Model.config_class.from_pretrained(
                    _config_source("facebook/dinov2-base"),
                ))
        else:
        ...
        self.image_preprocess_dino = AutoImageProcessor.from_pretrained(
            self.cfg.pretrained_dino_name_or_path
            if self.cfg.pretrained_dino_name_or_path is not None
            else _config_source("facebook/dinov2-base"),
        )
