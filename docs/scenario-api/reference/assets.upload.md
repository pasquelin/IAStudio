...
`client.assets.upload(AssetUploadParamsparams, RequestOptionsoptions?): AssetUploadResponse`
...
- `params: AssetUploadParams`
  ...
  - `name: string`
    ...
    Body param: The original file name of the image (example: "low-res-image.jpg").
  ...
  - `canvas?: string`
    ...
    Body param: The canvas to upload as a stringified JSON. Ignored if `image` is provided.

  - `collectionIds?: Array<string>`

    Body param: The IDs of the collections to add the asset to. If provided, the new asset will be added to the collections.

  - `hide?: boolean`

    Body param: Toggles the hidden status of the asset.

  - `image?: string`

    Body param: The image to upload in base64 format string.
  ...
  - `thumbnail?: string`
    ...
    Body param: The thumbnail for the canvas in base64 format string. Ignored if `image` is provided.
...
const response = await client.assets.upload({ name: 'name' });
