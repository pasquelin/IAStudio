...
`client.jobs.triggerAction(stringjobID, JobTriggerActionParamsparams, RequestOptionsoptions?): JobTriggerActionResponse`
...
- `params: JobTriggerActionParams`
  ...
  - `action?: "cancel"`
    ...
    Body param: The action to execute on the job, such as canceling it. Today only cancel on inference jobs is supported.

    - `"cancel"`

### Returns

- `JobTriggerActionResponse`
...
const response = await client.jobs.triggerAction('jobId');
