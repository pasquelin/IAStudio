...
`client.workflows.userApproval(stringworkflowID, WorkflowUserApprovalParamsparams, RequestOptionsoptions?): WorkflowUserApprovalResponse`

**put** `/workflows/{workflowId}/user-approval`

Respond to a user-approval node in a workflow. Pass action="approve" (default) to advance the workflow, or action="reject" to mark the node Rejected. A rejected stand-alone node cancels the workflow; a rejected ForEach iteration cascades to the surrounding loop.

### Parameters

- `workflowID: string`

- `params: WorkflowUserApprovalParams`
  ...
  - `nodeId: string`
    ...
    Body param: The ID of the user approval node to act on in the workflow

  - `workflowJobId: string`

    Body param: The ID of the workflow job that contains the user approval node

  - `action?: "approve" | "reject"`

    Body param: Decision taken on the approval node. "approve" marks it Success and advances the workflow; "reject" marks it Rejected and either cancels the workflow or — if the node is inside a ForEach iteration — cascades to the loop. Defaults to "approve" for backward compatibility.
...
const response = await client.workflows.userApproval('workflowId', {
  projectId: {},
  nodeId: 'nodeId',
