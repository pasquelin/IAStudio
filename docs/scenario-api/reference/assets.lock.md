...
`client.assets.lock(stringassetID, AssetLockParamsparams, RequestOptionsoptions?): AssetLockResponse`
...
- `AssetLockResponse`
  ...
  - `lockId?: string`
    ...
    The value of the lock to use when updating/unlocking the canvas.
...
const response = await client.assets.lock('assetId', { lockExpiresAt: 'lockExpiresAt' });
