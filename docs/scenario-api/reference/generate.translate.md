...
`client.generate.translate(GenerateTranslateParamsparams, RequestOptionsoptions?): GenerateTranslateResponse`
...
- `params: GenerateTranslateParams`
  ...
  - `prompt: string`
    ...
    Body param: The prompt to translate.
...
const response = await client.generate.translate({ prompt: 'prompt' });

console.log(response.detectedLanguage);
```
