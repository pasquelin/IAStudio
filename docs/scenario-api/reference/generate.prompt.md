...
`client.generate.prompt(GeneratePromptParamsparams, RequestOptionsoptions?): GeneratePromptResponse`
...
- `params: GeneratePromptParams`
  ...
  - `mode: "completion" | "contextual" | "contextual-v2" | 3 more`

    Body param: The mode used to generate new prompt(s).
  ...
  - `assetIds?: Array<string>`
    ...
    Body param: The asset ids used to condition the generation.
  ...
  - `images?: Array<string>`
    ...
    Body param: List of images used to condition the generation.
    ...
    A referenced asset ID may point to an image or a video. A video is used
    for visual conditioning via its first and last frames (a single frame when
    only one is available) and stays a single reference.
    ...
    - `contextual-v2` (and modes aliased to it) accepts up to 15 references; the other modes accept up to 5.

  - `modelId?: string`

    Body param: The modelId used to condition the generation.
  ...
  - `prompt?: string`
    ...
    Body param: The initial prompt spark feed to `completion`, `inventive` or `structured` modes.
...
- `GeneratePromptResponse`
  ...
  - `calls?: Array<Call>`
    ...
    The full ready-to-run model calls, index-aligned with `prompts`:
    `prompts[i]` is the prompt text of `calls[i]`.

    - `modelId: string`

      The target model the call is for.

    - `parameters: unknown`

      Schema-valid dynamic parameters for the target model, prompt included.

    - `rationale?: string`

      Optional rationale for the proposed call.
...
const response = await client.generate.prompt({ mode: 'completion' });
