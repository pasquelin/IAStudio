...
`client.generate.runModel(stringmodelID, GenerateRunModelParamsparams, RequestOptionsoptions?): GenerateRunModelResponse`

**post** `/generate/custom/{modelId}`

Generate with any model (Image, Video, Audio, 3d, Text).
...
- `params: GenerateRunModelParams`
  ...
  - `body: unknown`
    ...
    Body param: The request body for the custom generation must be retrieve from GET /models/{modelId} inputs fields
  ...
  - `ipDetection?: boolean`
    ...
    Query param: Set to "true" to run an IP-detection preflight on this request. Combine with ?dryRun=true for an advisory check that returns findings without generating.
...
const response = await client.generate.runModel('modelId', { body: {} });
