...
`client.workflows.list(WorkflowListParamsquery?, RequestOptionsoptions?): WorkflowsCursor<WorkflowListResponse>`
...
- `query: WorkflowListParams`
  ...
  - `privacy?: "public" | "private" | "unlisted"`
    ...
    The privacy of the workflows to return. The default value is `private`, possible values are `private`, `public` and `unlisted`. Default value is `private`.
  ...
  - `sortDirection?: "asc" | "desc"`
    ...
    Sort direction applied to `updatedAt`: `asc` or `desc` (default `desc`). Public listings are always ordered by `updatedAt`. Private listings honor it only when a `status` is provided; without a `status` this parameter is ignored.
  ...
  - `status?: "draft" | "ready"`
    ...
    The status of the workflows to return. Only available when privacy=private/unlisted. Default undefined value returns all statuses for private, `ready` for public.
