...
`engines/timeline/open-sink.ts` ouvre un `VideoSampleSink` mediabunny là où l'asset porte une piste
...
frame et en reçoit une. Les sinks ouverts sont bornés par le `DecoderPool`, une LRU par moteur —
un GPU grand public n'offre que deux à quatre décodeurs matériels.
