...
- **le signe pourcent** — `formatPercent`, dans le même fichier : le français pose une insécable
  avant le signe, l’anglais n’en pose aucune. Trois sites l’écrivaient à la main, deux à la
  française, et cette espace partait telle quelle vers un lecteur anglais. **Aucune garde i18n ne
  pouvait la voir** — un signe n’est pas un mot, donc aucun bundle ne le porte ;
  `no-composed-percent.test.ts` est la garde écrite pour ce trou-là ;
