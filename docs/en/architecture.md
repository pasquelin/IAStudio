...
- **the percent sign** — `formatPercent`, in the same file: French puts a no-break space before
  the sign, English puts none. Three sites wrote it by hand, two of them the French way, and that
  space shipped as-is to an English reader. **No i18n guard could see it** — a sign is not a word,
  so no bundle carries it; `no-composed-percent.test.ts` is the guard written for that gap;
