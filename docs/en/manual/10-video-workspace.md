...
**Drag an asset from the shelf and drop it on the timeline.**

The studio then chooses:

- **the track** — a sound goes on a sound track, everything else on a picture track. Locked and
  muted tracks are avoided, because a drop that lands there looks like it did nothing;
- **the duration** — that of the media. A still image, or a medium whose duration is unknown, lasts
  **5 seconds** by default;
- **the position** — aligned to a whole frame, never between two.
...
[Assets](07-assets.md#if-ffmpeg-is-missing).
