...
import { cutMontage, generated, madeBoat, madeCar, madeChest, scene } from './setups'

/**
 * Sections 20 to 23: generating, generating FROM something of the project, and stitching several
 * documents together.
 *
 * The bench answers a generation synchronously — the job lands `success` with its asset already
 * in the catalogue. Measuring patience is not the point; measuring whether the model armed a
 * model, filled the form and SENT it is.
 */

/** A generated chest already placed in the scene, which 22.5 and 22.6 move and scale. */
const chestIn = (studio: FakeStudio): void => {
  madeChest(studio)
  const made = studio.bench().assets.at(-1)?.id ?? ''
  studio.run('node.addModel', { assetId: made, name: 'Coffre' })
}
...
export const GENERATION_SCENARIOS: readonly Scenario[] = [
  ...
  },
  {
    ...
    passed: run => {
      ...
      return chest !== undefined && (chest.position.z !== 0 || chest.position.x !== 0)
    },
  },
  {
    ...
    passed: run => (read.nodeNamed(run, 'Coffre')?.scale.x ?? 1) !== 1,
  },
  ...
  },
  {
    ...
    setup: scene(),
    passed: run => {
      const scene3d = read.inSpace(run, '3d')[0]
      return scene3d?.world.environment !== null && read.nodesOfKind(run, 'model').length >= 1
    },
  },
  {
    ...
    setup: studio => {
      scene()(studio)
      chestIn(studio)
      studio.run('node.add', { kind: 'box', name: 'Autre' })
    ...
    passed: run =>
      read.nodeNamed(run, 'Coffre')?.textures.map !== undefined &&
      read.nodeNamed(run, 'Autre')?.textures.map === undefined,
  },
]
