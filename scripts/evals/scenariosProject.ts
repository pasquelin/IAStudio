 ...
 * it, and making documents in it.
 *
 * The read-only ones are scored on the studio being UNTOUCHED and a sentence having been said —
...
export const PROJECT_SCENARIOS: readonly Scenario[] = [
  // ——— 1. Compréhension du projet — lecture seule ———
  {
    ...
    setup: scene('Scène 1'),
    passed: run => idle(run) && read.answeredWith(run, 'studio.state'),
  },
  asking(
    '1.2 lists the project files by type',
    'Liste-moi les fichiers présents dans mon projet, classés par type.',
    run =>
      idle(run) && (read.answeredWith(run, 'files.list') || read.answeredWith(run, 'files.search')),
  ),
  asking(
  ...
  },
  {
    ...
    setup: cutMontage,
    passed: run => idle(run) && read.answeredWith(run, 'sequence.state'),
  },
  {
  ...
  ),
  {
    ...
    passed: run => read.front(run)?.space === '3d',
  },
  ...
  ),
  {
    ...
    setup: studio => {
      studio.run('file.open', { path: '3D/a medieval stone castle with towers.glb' })
    },
  ...
  },
  {
    ...
    setup: studio => {
      namedCopy(studio)
      studio.run('files.move', {
        paths: ['Images/bateau-test.png'],
  ...
  },
  {
    ...
    setup: studio => {
      scene('Test MCP')(studio)
      studio.run('workspace.open', { workspace: 'image' })
    },
